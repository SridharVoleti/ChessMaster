...
export async function GET(req: NextRequest) {
  try {
    ...
    const session = getAuthzService().getActiveSession(student.id)
    if (!session) {
...
export async function POST(req: NextRequest) {
  try {
    const student = studentFromCookies(req.cookies)
    if (!student) return unauthenticatedResponse()
    const { session, resumed } = getAuthzService().startSession(student.id)
...
export async function DELETE(req: NextRequest) {
  try {
    const student = studentFromCookies(req.cookies)
    if (!student) return unauthenticatedResponse()
    const session = getAuthzService().endSession(student.id)
    return NextResponse.json({ session })
  } catch (e) {
    return errorResponse(e)
  }
